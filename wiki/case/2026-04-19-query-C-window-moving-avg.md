...
- Exchange #1의 결과: data가 **`(agent_gu, deal_year, deal_month)`** 튜플의
  해시로 partition되어 있다.
- Window가 요구하는 것: **`agent_gu`** 단독 기준 partition. 같은
  `agent_gu`의 모든 row가 한 partition에 모여야 PARTITION BY가 성립.
- 문제: `hashpartitioning(a, b, c)`의 해시는 `(a, b, c)` 튜플 전체로 계산
  되므로, 같은 `a`이라도 `(b, c)`가 다르면 서로 다른 partition에 갈 수
  있다.
- Spark의 distribution 추론은 "`(a, b, c)`의 해시 분포가 `a` 단독의 해시
  분포와 compatible함"을 일반적으로 보장하지 않는다.
- 결론: `EnsureRequirements` rule이 **추가 Exchange를 강제 삽입**.
...
같은 키의 **prefix** (여기서는 `agent_gu`가 `(agent_gu, deal_year,
deal_month)`의 prefix)라도 기존 hashpartitioning을 재사용할 수 **없다**.
해시 bucket 내부에 같은 prefix 값의 row가 섞여 있어도 다른 bucket에도
같은 prefix 값이 있을 수 있기 때문.

> **예외**: Spark에는 `HashPartitioning(a, b, c).satisfies(Distribution(a))`
> 를 true로 리턴하는 특수 경로가 일부 있지만, 기본 `ClusteredDistribution`
> 요구는 "정확한 key 집합 일치"를 요구한다. 쿼리 C의 Window distribution 요구는
> 정확히 `(agent_gu)`이므로 `(agent_gu, deal_year, deal_month)` 분포는
> compatible하지 않다고 판단.
